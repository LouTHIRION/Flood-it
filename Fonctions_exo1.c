...
int trouve_zone_imp(int **M, int nbcase, int i, int j, int *taille, ListeCase *L){
	...
	while(!test_liste_vide(p)) {
		...
		ajoute_en_tete(L, i, j);
		*taille += 1;
		if(i<nbcase-1 && !test_est_dans(i+1, j, p) && cl == M[i+1][j] && !test_est_dans(i+1, j, L)) {
			ajoute_en_tete(p, i+1, j);
		}
		if(i>0 && !test_est_dans(i-1, j, p) && cl == M[i-1][j] && !test_est_dans(i-1, j, L) ) {
			ajoute_en_tete(p, i-1, j);
		}
		if(j<nbcase-1 && !test_est_dans(i, j+1, p) && cl == M[i][j+1] && !test_est_dans(i, j+1, L) ) {
			ajoute_en_tete(p, i, j+1);
		}
		if(j>0 && !test_est_dans(i, j-1, p) && cl == M[i][j-1] && !test_est_dans(i, j-1, L) ) {
			ajoute_en_tete(p, i, j-1);
		}
	}
}
